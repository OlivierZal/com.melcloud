...
class MelCloudDevice extends Homey.Device {
    ...
    async onCapabilityFanSet (value, opts){
        try{
            console.log("FAN SET")
            if (value == 0){
                console.log("autoooo fan")
                let options = {
                    excerpt: 'Fan Speed set to AUTO'
                }
                let noti = new Homey.Notification(options)
            ManagerNotifications.registerNotification(noti) 
            }
        ...
        let modether
             if (onoff == false){
                ...
                if (mode == "cool"){
                    ...
                    modether = "auto"
                    if (mode !== "dry" && mode !=="fan") mode = "auto"
                } 
            }
            this.setCapabilityValue('thermostat_mode', modether) 
            this.setCapabilityValue('mode_capability', mode)
            return modether
    }
