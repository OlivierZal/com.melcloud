...
class MelCloudDevice extends Homey.Device {
    ...
    getdevicedata(data, callback){
        ...
        http.get(askdevice).then(function (result) {
            ...
            if (result.data) {
                ...
                console.log("old: ", forcedold)
                console.log("new: ", result.data.ForcedHotWaterMode)
                let igual = forcedold == result.data.ForcedHotWaterMode
                console.log("igualgetdata: ", igual)
    ...
    async onSettings(){
        try{
            this._syncTimeout = setTimeout(this.updateCapabilityValues.bind(this), (1000));
        }catch (err) {
            throw new Error(err);
        }
    }
