...
export interface SmartFanConfig {
  ...
  readonly sensorTimeoutMinutes: number
}
...
const DEFAULT_SENSOR_TIMEOUT_MINUTES = 5
...
export const DEFAULT_SMART_FAN_CONFIG: SmartFanConfig = {
  ...
  mode: 'moderate',
  sensorTimeoutMinutes: DEFAULT_SENSOR_TIMEOUT_MINUTES,
}
...
export const calculateOptimalFanSpeed = ({
  ...
  // Check hysteresis (skip for user-initiated changes like temperature adjustments)
  if (!shouldSkipHysteresis && now - state.lastChangeTime < hysteresisMs) {
    return { action: 'none' }
  }
