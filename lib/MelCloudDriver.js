...
class MelCloudDriver extends Homey.Driver {
    ...
    onInit() {
		...
		this._ThermostatModeTrigger.registerRunListener((args, state) => {
            let conditionMet = args.mode_capability_action == args.device.getCapabilityValue('mode_capability');
		...
		this._HorizontalSwingTrigger.registerRunListener((args) => {
            let conditionMet = args.horizontal_swing_action == args.device.getCapabilityValue('horizontal');
            return Promise.resolve(conditionMet);
        })
		...
		this._ThermostatModeCondition.registerRunListener((args, state) => {
            console.log()
            let conditionMet = args.mode_capability_condition == args.device.getCapabilityValue('mode_capability');
            return Promise.resolve(conditionMet);
		});
