...
npm run typecheck    # type-check with tsgo (fast, native compiler)
npm run lint         # eslint (TS, HTML, CSS, JSON, YAML, Markdown)
npm run build        # bundle browser entries (esbuild) + compile with tsgo
...
- Both the build and `npm run typecheck` use `tsgo` (the native TypeScript compiler preview) for speed.
