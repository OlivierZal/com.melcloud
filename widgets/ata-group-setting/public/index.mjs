...
const generateStyleNumber = ({ divisor, gap, min, multiplier, }) => ((Math.random() * gap + min) * (multiplier ?? DEFAULT_MULTIPLIER_ONE)) /
    ((divisor ?? DEFAULT_DIVISOR_ONE) || DEFAULT_DIVISOR_ONE);
class SmokeParticle {
    opacity = generateStyleNumber({ gap: 0.05, min: 0.05 });
    positionY;
    size = generateStyleNumber({ gap: 2, min: 2 });
    #context;
    #speedX;
    #speedY;
    #positionX;
    ...
    draw() {
        ...
        this.#context.arc(this.#positionX, this.positionY, this.size, START_ANGLE, FULL_CIRCLE);
...
const getButtonElement = (id) => getElement(id, HTMLButtonElement, 'button');
const getCanvasElement = (id) => getElement(id, HTMLCanvasElement, 'canvas');
...
let ataCapabilities = [];
let defaultAtaValues = {};
let smokeAnimationFrameId = null;
let smokeParticles = [];
const createAnimationMapping = () => {
    let flameIndex = 0;
    let leafIndex = 0;
...
const getZoneName = (name, level) => `${'···'.repeat(level)} ${name}`;
const getZonePath = () => zoneElement.value.replace('_', '/');
const generateStyleString = (params, unit = '') => `${String(generateStyleNumber(params))}${unit}`;
const generateDelay = (delay, speed) => (Math.random() * delay) /
    (SPEED_FACTOR_MIN *
...
const createLabelElement = (valueElement, text) => {
    const labelElement = document.createElement('label');
    labelElement.classList.add('label', 'text-default', 'text-color', 'font-normal');
    ({ id: labelElement.htmlFor } = valueElement);
    labelElement.textContent = text;
    labelElement.append(valueElement);
    return labelElement;
};
const createValueElement = (parentElement, { title, valueElement, }) => {
    if (valueElement) {
        parentElement.append(createLabelElement(valueElement, title));
    }
};
const handleNumericInputElement = (inputElement, { max, min }) => {
...
const createInputElement = ({ id, max, min, placeholder, type, value, }) => {
    const inputElement = document.createElement('input');
    inputElement.classList.add('input', 'input-ghost', 'text-default', 'text-light', 'font-normal');
    inputElement.id = id;
...
const createSelectElement = (homey, id, values) => {
    const selectElement = document.createElement('select');
    selectElement.classList.add('select', 'select-ghost', 'text-default', 'text-light', 'font-normal');
    selectElement.id = id;
    for (const option of [
        { id: '', label: '' },
        ...(values ??
...
const createAnimatedElement = (name) => {
...
const isGroupAtaState = (value) => value in defaultAtaValues;
const buildAtaValuesBody = () => Object.fromEntries(
// eslint-disable-next-line unicorn/prefer-spread
Array.from(ataValuesElement.querySelectorAll('input, select'))
    .filter(({ id, value }) => isGroupAtaState(id) &&
    !['', zoneMapping[zoneElement.value]?.[id]?.toString()].includes(value))
    .map((element) => [element.id, processValue(element)]));
const updateZoneMapping = (data) => {
    const { value } = zoneElement;
    zoneMapping[value] = { ...zoneMapping[value], ...data };
};
const updateAtaValue = (id) => {
...
const refreshAtaValues = () => {
    for (const [ataKey] of ataCapabilities) {
        updateAtaValue(ataKey);
    }
};
const getPreviousElement = (name, index) => document.querySelector(`#${name}-${String(Number(index) - INCREMENT_ONE)}`);
const createSmoke = (flame, speed) => {
    if (flame.isConnected && canvasContext) {
        const { left, top, width } = flame.getBoundingClientRect();
        let index = 0;
        while (index <= SMOKE_ITERATIONS) {
            smokeParticles.push(new SmokeParticle(canvasContext, left + width / FACTOR_TWO, top - Number.parseFloat(getComputedStyle(flame).insetBlockEnd)));
            index += INCREMENT_ONE;
        }
        setTimeout(() => {
...
const generateSmoke = (speed) => {
    if (canvasContext) {
        ;
        ({ innerHeight: canvas.height, innerWidth: canvas.width } = globalThis);
        canvasContext.clearRect(DEFAULT_RECT_X, DEFAULT_RECT_Y, canvas.width, canvas.height);
        smokeParticles = smokeParticles.filter((particle) => {
            particle.update(speed);
            particle.draw();
            return (particle.size > SMOKE_PARTICLE_SIZE_MIN &&
...
const generateFlameAnimation = (flame, speed) => {
    const animation = flame.animate([...Array.from({ length: 101 }).keys()].map(() => {
...
const createFlame = (speed) => {
    ...
    if (name !== undefined) {
        ...
        const previousLeft = previousElement ?
        ...
        flame.style.insetInlineStart = generateStyleString({
            gap: FLAME_GAP,
            min: previousLeft > window.innerWidth ?
                -FLAME_GAP
...
const generateFlames = (speed) => {
    animationTimeouts.push(setTimeout(() => {
        createFlame(speed);
        generateFlames(speed);
    }, generateDelay(FLAME_DELAY, speed)));
};
const handleFireAnimation = (speed) => {
    generateFlames(speed);
    generateSmoke(speed);
};
const generateSnowflakeAnimation = (snowflake, speed) => {
...
const createSnowflake = (speed) => {
    ...
    if (name !== undefined) {
        ...
        snowflake.style.fontSize = generateStyleString({ divisor: speed, gap: 1, min: 2 }, 'rem');
...
const generateSnowflakes = (speed) => {
    animationTimeouts.push(setTimeout(() => {
        createSnowflake(speed);
        generateSnowflakes(speed);
    }, generateDelay(SNOWFLAKE_DELAY, speed)));
};
const handleSnowAnimation = (speed) => {
    generateSnowflakes(speed);
};
const generateSunExitAnimation = (sun) => {
    const duration = Number(sunAnimation.enter?.currentTime ?? SUN_ENTER_AND_EXIT_DURATION);
...
const generateSunEnterAnimation = (sun) => {
    const duration = Number(sunAnimation.exit?.currentTime ?? SUN_ENTER_AND_EXIT_DURATION);
    sunAnimation.exit?.pause();
    sunAnimation.exit = null;
    const { blockSize, inlineSize, insetBlockStart, insetInlineEnd } = getComputedStyle(sun);
    const animation = sun.animate([
        {
            insetBlockStart: `${String(Number.parseFloat(insetBlockStart))}px`,
            insetInlineEnd: `${String(Number.parseFloat(insetInlineEnd))}px`,
        ...
        {
            insetBlockStart: `${String((window.innerHeight - Number.parseFloat(blockSize)) / FACTOR_TWO)}px`,
            insetInlineEnd: `${String((window.innerWidth - Number.parseFloat(inlineSize)) / FACTOR_TWO)}px`,
        },
    ], { duration, easing: 'ease-in-out', fill: 'forwards' });
    animation.onfinish = () => {
        sunAnimation.enter = null;
...
const generateSunShineAnimation = (sun) => {
    const animation = sun.animate([
...
const handleSunAnimation = (speed) => {
...
const generateLeafAnimation = (leaf, speed) => {
    ...
    const animation = leaf.animate([...Array.from({ length: 101 }).keys()].map((index) => {
        const angle = ((index - loopStart) / loopDuration) * FULL_CIRCLE;
        const indexLoopRadius = index >= loopStart && index < loopEnd ? loopRadius : LEAF_NO_LOOP_RADIUS;
        const oscillate = indexLoopRadius > LEAF_NO_LOOP_RADIUS ?
        ...
        const translateX = `${String(index * FACTOR_FIVE + indexLoopRadius * Math.sin(angle))}px`;
        const translateY = `${String(-(index * FACTOR_TWO - indexLoopRadius * Math.cos(angle)))}px`;
        return {
...
const generateLeaves = (speed) => {
    animationTimeouts.push(setTimeout(() => {
        createLeaf(speed);
        generateLeaves(speed);
    }, generateDelay(LEAF_DELAY, speed)));
};
const handleWindAnimation = (speed) => {
    generateLeaves(speed);
};
const getAtaValues = async (homey) => homeyApi(homey, `/values/ata/${getZonePath()}`);
const getDetailedAtaValues = async (homey) => homeyApi(homey, `/values/ata/${getZonePath()}?${new URLSearchParams({
    mode: 'detailed',
    status: 'on',
})}`);
const getModes = async (homey) => {
    const detailedAtaValues = await getDetailedAtaValues(homey);
    return detailedAtaValues.OperationMode;
};
const resetFireAnimation = async (homey, resetParams) => {
    if (resetParams) {
    ...
    // eslint-disable-next-line unicorn/prefer-spread
    for (const flame of Array.from(document.querySelectorAll('.flame'))) {
        setTimeout(() => {
            flame.remove();
        }, generateDelay(FLAME_DELAY, SPEED_VERY_SLOW));
    }
};
const resetSunAnimation = async (homey, resetParams) => {
    const sun = document.querySelector('#sun-1');
    const modes = await getModes(homey);
    if (sun &&
        ...
        (!resetParams ||
            ...
            (resetParams.mode !== MODE_DRY &&
                (resetParams.mode !== MODE_MIXED ||
                    modes.every((currentMode) => currentMode !== MODE_DRY))))) {
        sunAnimation.exit = generateSunExitAnimation(sun);
    }
};
const resetAnimation = async (homey, resetParams) => {
...
const handleMixedAnimation = async (homey, speed) => {
...
const hasModeAnimation = (mode) => mode in animationHandling;
const handleAnimation = async (homey, state, isAnimations) => {
    if (isAnimations) {
...
const fetchAtaValues = async (homey, isAnimations) => {
...
const generateAtaValue = (homey, { id, type, values, }) => {
...
const generateZones = async (zones = []) => {
    if (zones.length) {
...
const fetchAtaCapabilities = async (homey) => {
    ataCapabilities = await homeyApi(homey, '/capabilities/ata');
    defaultAtaValues = Object.fromEntries(ataCapabilities.map(([ataKey]) => [ataKey, null]));
};
const setAtaValues = async (homey) => {
    try {
        const body = buildAtaValuesBody();
        if (Object.keys(body).length) {
            await homey.api('PUT', `/values/ata/${getZonePath()}`, body);
        }
    }
    catch { }
};
const addEventListeners = (homey, isAnimations) => {
...
const fetchBuildings = async (homey) => {
    const buildings = await homeyApi(homey, `/buildings?${new URLSearchParams({
        type: '0',
    })}`);
    if (buildings.length) {
        const { animations: isAnimations, default_zone: defaultZone } = homey.getSettings();
