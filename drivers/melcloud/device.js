...
class MELCloudDeviceAta extends Homey.Device {
  ...
  async syncDataFromDevice() {
    ...
    try {
      ...
      await http.get(options).then(async (result) => {
        ...
        // Fan speed
        const currentFanSpeed = this.getCapabilityValue('fan_power');
        ...
        // Horizontal vane direction
        const currentHorizontal = this.getCapabilityValue('horizontal');
        ...
        const thermostatMode = await this.setThermostatMode(operationMode, result.data.Power);
        if (!thermostatMode) {
          await this.setCapabilityValue('thermostat_mode', 'off')
            .then(this.log(`\`${this.getName()}\`: capability \`thermostat_mode\` has been set (off)`))
        ...
        // Vertical vane direction
        const currentVertical = this.getCapabilityValue('vertical');
  ...
  // Sync `onoff`, `operation_mode` and `thermostat_mode` from `thermostat_mode`
  async setOperationMode(thermostatMode) {
    ...
    if (thermostatMode === 'off') {
      ...
      await this.setCapabilityValue('onoff', true)
        .then(this.log(`\`${this.getName()}\`: capability \`onoff\` has been set (true)`))
        .catch((error) => this.error(`\`${this.getName()}\`: capability \`onoff\` has not been set (${error})`));
      if (thermostatMode === 'heat') {
        await this.setCapabilityValue('operation_mode', 'heat')
          .then(this.log(`\`${this.getName()}\`: capability \`operation_mode\` has been set (heat)`))
          .catch((error) => this.error(`\`${this.getName()}\`: capability \`operation_mode\` has not been set (${error})`));
      } else if (thermostatMode === 'cool') {
        await this.setCapabilityValue('operation_mode', 'cool')
          .then(this.log(`\`${this.getName()}\`: capability \`operation_mode\` has been set (cool)`))
          .catch((error) => this.error(`\`${this.getName()}\`: capability \`operation_mode\` has not been set (${error})`));
      } else if (thermostatMode === 'auto') {
        await this.setCapabilityValue('operation_mode', 'auto')
          .then(this.log(`\`${this.getName()}\`: capability \`operation_mode\` has been set (auto)`))
          .catch((error) => this.error(`\`${this.getName()}\`: capability \`operation_mode\` has not been set (${error})`));
      }
    }
  }

  // Sync `onoff`, `operation_mode` and `thermostat_mode` from `operation_mode` and `onoff`
